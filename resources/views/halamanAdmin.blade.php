...
<div class="haladmin">
    ...
    <div class="tab">
        <button style="font-size:24px;" class="tablinks" onclick="openCity(event, 'transaksi')" id="defaultOpen">Konfirmasi Transaksi</button>
        <button style="font-size:24px;" class="tablinks" onclick="window.location.href='/halaman-admin/profesi'">Pendaftaran Percetakaan</button>
    </div>
<!-- Tab content -->
<div id="transaksi" class="tabcontent">
  ...
  <div style="border: 1px solid #ddd;width:16.66%"><center>{{$transaksis[$i]->nama}}</center></div>
      ...
      <div style="border: 1px solid #ddd;width:16.66%"><center>{{$transaksis[$i]->jumlah+$transaksis[$i]->kode_unik}}</center></div>
  <div style="border: 1px solid #ddd;width:16.66%"><center><img style="width:100px" src="/{{$transaksis[$i]->gambar_konfirmasi}}"</center></div>
      <div style="border: 1px solid #ddd;width:16.66%"><center>
            <form action='/tolak-transaksi?id={{$transaksis[$i]->id}}' method="post">
                {{csrf_field()}}
                <button type="submit" class="tolak">Tolak</button>
            </form>
            <form action='/terima-transaksi?id={{$transaksis[$i]->id}}' method="post">
                {{csrf_field()}}
